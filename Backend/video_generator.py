...
class AudioManager:
    def __init__(self):
        ...
        try:
            ...
            print(
                colored(
                    f"⚠️ Could not initialize TTS model during startup: {str(e)}",
                    "yellow"))
            print(
                colored(
                    f"ℹ️ TTS model will be initialized on first use",
                    "blue"))
        ...
        try:
            ...
            if hasattr(self.tts_model, "speakers") and self.tts_model.speakers:
                ...
                for speaker in self.all_speakers:
                    if any(en_indicator in speaker.lower()
                           for en_indicator in ["en_", "en-", "english"]):
                        self.english_speakers.append(speaker)

                # If no obvious English speakers found, look for ones with "en"
                # language
                if not self.english_speakers and hasattr(
                        self.tts_model, "languages") and "en" in self.tts_model.languages:
                    # Based on our analysis, try these specific speakers that
                    # seem to handle English well
                    english_candidates = [
                        "Ana Florence",
                        "Brenda Stern",
                        "Henriette Usha",
                ...
                if not self.english_speakers and "en" in self.tts_model.languages:
                    log_info(
                        "No specific English speakers found, using any speaker with English language")
                    self.english_speakers = self.all_speakers

                log_info(
                    f"Found {len(self.english_speakers)} suitable English speakers")

                # Simple gender classification - better than before but still
                # basic
                for speaker in self.all_speakers:
                ...
                log_info(
                    f"Classified {len(self.male_speakers)} male voices and {len(self.female_speakers)} female voices")
                log_info(f"Total available voices: {len(self.all_speakers)}")

            # Store available languages
            if hasattr(
                    self.tts_model,
                    "languages") and self.tts_model.languages:
    ...
    def select_coqui_voice(self, channel_type, gender=None):
        ...
        emotion_options = self.emotion_map.get(
            channel_type, self.emotion_map['default'])
        ...
        if gender == 'male' and self.male_speakers:
            ...
            english_male = [
                s for s in self.male_speakers if s in self.english_speakers]
            if english_male:
                voice_candidates = english_male
                log_info(
                    f"Using English male voice pool ({len(voice_candidates)} voices)")
            else:
                voice_candidates = self.male_speakers
                log_info(
                    f"Using any male voice pool ({len(voice_candidates)} voices)")
        elif gender == 'female' and self.female_speakers:
            # Try to find English female speakers first
            english_female = [
                s for s in self.female_speakers if s in self.english_speakers]
            if english_female:
                voice_candidates = english_female
                log_info(
                    f"Using English female voice pool ({len(voice_candidates)} voices)")
            else:
                voice_candidates = self.female_speakers
                log_info(
                    f"Using any female voice pool ({len(voice_candidates)} voices)")
        elif self.english_speakers:
            # No gender preference or no voices for that gender, use any
            # English speaker
            voice_candidates = self.english_speakers
            log_info(
                f"Using any English voice pool ({len(voice_candidates)} voices)")
        else:
            # Fallback to any available speaker
            voice_candidates = self.all_speakers
            log_info(
                f"Using any available voice pool ({len(voice_candidates)} voices)")
    ...
    async def generate_tts_coqui(
        ...
        try:
            ...
            if not tagged_sentences:
                ...
                tagged_sentences = [(s.strip(), global_style)
                                    for s in sentences if s.strip()]

            log_info(
                f"Processing {len(tagged_sentences)} sentences with emotion for better TTS quality")
            ...
            for i, (sentence, sentence_emotion) in enumerate(tagged_sentences):
                log_step(
                    ...
                    f"Generating speech chunk {i+1}/{len(tagged_sentences)} with emotion: {sentence_emotion}")
                ...
                try:
                    # Use the voice parameter directly - it should be a valid
                    # speaker name
                    ...
                    log_error(
                        f"Failed to generate TTS for chunk {i+1}: {str(chunk_error)}")
                    ...
                    try:
                        ...
                        log_error(
                            f"Fallback TTS also failed: {str(fallback_error)}")
                        continue
    ...
    def _enhance_text_for_emotion(self, text, emotion):
        ...
        if emotion == "excited" or emotion == "energetic":
            ...
            for i, word in enumerate(words):
                if len(word) > 4 and random.random(
                ) < 0.3:  # Randomly emphasize some longer words
    ...
    async def generate_tts_openai(self, text, voice="nova"):
        ...
        try:
            ...
            output_path = os.path.join(
                self.voiceovers_dir,
                f"openai_{voice}_{int(time.time())}.mp3")
            ...
            print(
                colored(
                    f"✓ Generated TTS audio with OpenAI: {output_path}",
                    "green"))
            return output_path
...
class VideoGenerator:
    def __init__(self):
        ...
        print(
            colored(
                f"Background music: {'Enabled' if self.use_background_music else 'Disabled'}",
                "blue"))
    ...
    def generate_video_for_channel(self, channel, topic, hashtags):
        ...
        try:
            ...
            success, script = self.script_generator.generate_script(
                topic, channel)
            if not success:
            ...
            if not self.is_duration_valid(estimated_duration, channel):
                print(
                    colored(
                        f"Estimated duration ({estimated_duration}s) not suitable",
                        "red"))
                return None
            ...
            is_complete, missing = self.validator.check_content_completeness(
                video_data)
            if not is_complete:
                print(
                    colored(
                        f"Missing content elements: {', '.join(missing)}",
                        "red"))
                return None
    ...
    def get_voice_for_channel(self, channel_type):
        ...
        # OpenAI voices: 'nova', 'shimmer', 'echo', 'onyx', 'fable', 'alloy',
        # 'ash', 'sage', 'coral'
        voices = {
        ...
        # Default to nova if channel not found
        return voices.get(channel_type, 'nova')
    ...
    async def create_video(self, script_file, channel_type, output_path=None):
        ...
        try:
            ...
            try:
                ...
                # Check if we have a plain text version of the script (which
                # might be better formatted)
                    ...
                    try:
                        with open(txt_script_file, 'r', encoding='utf-8') as f:
                            ...
                            if txt_script:
                                print(
                                    colored(
                                        "Using plain text version of script for better formatting",
                                        "green"))
                                script = txt_script
                    except Exception as e:
                        print(
                            colored(
                                f"Error loading plain text script: {str(e)}",
                                "yellow"))
                ...
                print(
                    colored(
                        f"Script to be used for video generation:\n{script}",
                        "blue"))
            ...
            if self.use_background_music:
                ...
                if background_music:
                    print(
                        colored(
                            f"Selected background music for {channel_type}",
                            "green"))
            ...
            # Only keep 20 videos per channel, and always keep videos newer
            # than 30 days
            self.cleanup_video_library(
                channel_type, max_videos=20, days_to_keep=30)
    ...
    async def _generate_tts(self, script, channel_type):
        ...
        try:
            ...
            if not clean_script.strip():
                print(
                    colored(
                        "Warning: Script is empty after cleaning. Cannot generate TTS.",
                        "red"))
                return None
            ...
            if self.use_coqui_tts:
                try:
                    ...
                    voice, emotion = self.audio_manager.select_coqui_voice(
                        channel_type)
                    print(
                        colored(
                            f"Selected Coqui voice: {voice} (emotion: {emotion})",
                            "green"))
                    ...
                    if channel_type == 'tech_humor':
                        ...
                        print(
                            colored(
                                f"Using speed multiplier of {speed_multiplier} for {channel_type} content",
                                "blue"))
                        ...
                        print(
                            colored(
                                f"Using speed multiplier of {speed_multiplier} for {emotion} {channel_type} content",
                                "blue"))
                    ...
                    if tts_path:
                        print(
                            colored(
                                f"✓ Generated TTS using Coqui with voice: {voice}, emotion: {emotion}",
                                "green"))
                        return tts_path
                    else:
                        print(
                            colored(
                                "Failed to generate TTS with Coqui, falling back to OpenAI",
                                "yellow"))
                except Exception as e:
            ...
            if tts_path:
                print(
                    colored(
                        f"✓ Generated TTS using OpenAI with voice: {voice}",
                        "green"))
                return tts_path
            else:
    ...
    def _clean_script_for_tts(self, script):
        ...
        # Also catch if there's whitespace after
    ...
    def _enhance_script_with_emotion(self, script):
        ...
        sentences = [''.join(i) for i in zip(
            sentences[0::2], sentences[1::2] + [''] * (len(sentences[0::2]) - len(sentences[1::2])))]
    ...
    def _analyze_script_content(self, script):
        ...
        try:
            ...
            topics = {
                # Primary topic (e.g., 'coffee', 'debugging')
                'main_subject': [],
                # What's happening (e.g., 'drinking', 'coding')
                'actions': [],
                # Where it happens (e.g., 'office', 'desk')
                'environment': [],
                # Related objects (e.g., 'keyboard', 'monitor')
                'objects': [],
                # Emotional context (e.g., 'funny', 'serious')
                'mood': []
            }
    ...
    def _generate_search_terms(self, script_analysis):
        ...
        try:
            ...
            print(
                colored(
                    f"Generated search terms: {search_terms[:10]}",
                    "blue"))
    ...
    async def _get_video_suggestions(self, script):
        ...
        try:
            # Estimate duration based on word count (assuming ~3 words per
            # second)
            ...
            for line in response.split('\n'):
                if line.strip() and line[0].isdigit():
                    ...
                    suggestions.append({'term': term, 'explanation': line.split(
            ...
            for suggestion in suggestions:
                print(
                    colored(
                        f"• {suggestion['term']} - {suggestion['explanation']}",
                        "cyan"))
    ...
    async def _process_background_videos(self, channel_type, script=None):
        ...
        try:
            ...
            categories = [
                cat for cat in list(
                    dict.fromkeys(categories)) if cat]

            print(
                colored(
                    f"Identified categories: {', '.join(categories)}",
                    "blue"))
            ...
            for category in categories:
                category_dir = os.path.join(
                    categorized_dir, self._sanitize_filename(category))
                if os.path.exists(category_dir):
                    category_videos = [
                        os.path.join(
                            category_dir, f) for f in os.listdir(category_dir) if f.endswith(
                            ('.mp4', '.mov')) and os.path.getsize(
                            os.path.join(
                                category_dir, f)) > 0]
                    ...
                    for video_path in category_videos:
                        try:
                            ...
                            print(
                                colored(
                                    f"Error checking video {video_path}: {str(e)}",
                                    "yellow"))

            if categorized_videos:
                print(
                    colored(
                        f"Found {len(categorized_videos)} relevant categorized videos",
                        "green"))
                # If we have more than needed, randomly select some
                if len(categorized_videos) > target_video_count:
                    selected = random.sample(
                        categorized_videos, target_video_count)
                    final_videos.extend(selected)
                    print(
                        colored(
                            f"Selected {len(selected)} categorized videos",
                            "green"))
                else:
            ...
            if len(final_videos) < target_video_count:
                ...
                channel_videos = [
                    os.path.join(
                        video_dir, f) for f in os.listdir(video_dir) if f.endswith(
                        ('.mp4', '.mov')) and os.path.getsize(
                        os.path.join(
                            video_dir, f)) > 0]
                ...
                if valid_channel_videos:
                    ...
                    if len(valid_channel_videos) > remaining_slots:
                        selected = random.sample(
                            valid_channel_videos, remaining_slots)
                        final_videos.extend(selected)
                        print(
                            colored(
                                f"Added {len(selected)} channel-specific videos",
                                "green"))
                    else:
                        final_videos.extend(valid_channel_videos)
                        print(
                            colored(
                                f"Added {len(valid_channel_videos)} channel-specific videos",
                                "green"))
            ...
            if len(final_videos) < target_video_count and script:
                ...
                print(
                    colored(
                        f"Need {remaining_slots} more videos, downloading from Pexels",
                        "blue"))
                ...
                if suggestions:
                    # Generate search terms from suggestions and script
                    # analysis
                    search_terms = []
                    ...
                    for suggestion in suggestions:
                        if isinstance(
                                suggestion, dict) and 'term' in suggestion:
                    ...
                    analysis_terms = self._generate_search_terms(
                        script_analysis)
                    ...
                    # Search for videos and save them to categorized
                    # directories
                    for term in search_terms:
                        # Create category directory
                        category_dir = os.path.join(
                            categorized_dir, self._sanitize_filename(term))
                        ...
                        if videos:
                            ...
                            print(
                                colored(
                                    f"Downloaded and categorized video for '{term}'",
                                    "green"))
            ...
            # Step 5: If we still don't have enough videos, use generic search
            # terms
            if len(final_videos) < 2 and script:
                print(
                    colored(
                        "Not enough videos, searching for more from Pexels",
                        "yellow"))
            ...
            if len(final_videos) >= 2:
                print(
                    colored(
                        f"Using {len(final_videos)} videos for a dynamic video",
                        "green"))
                return final_videos
            ...
            print(
                colored(
                    "No videos found, creating default background",
                    "yellow"))
            ...
            print(
                colored(
                    f"Error processing background videos: {str(e)}",
                    "red"))
            traceback.print_exc()
    ...
    def _generate_generic_search_terms(self, channel_type):
        ...
        return generic_terms.get(
            channel_type, [
                'abstract background', 'colorful motion', 'dynamic background'])
    ...
    def _create_default_background(self, channel_type, duration=15):
        ...
        try:
            ...
            colors = {
                ...
                'fitness_motivation': ['#355c7d', '#6c5b7b']
            }
            ...
            txt_clip = txt_clip.set_position(
                ('center', 1700)).set_duration(duration)

            # Combine clips
            final_clip = CompositeVideoClip(
                [clip, txt_clip], size=(1080, 1920))

            # Detect number of CPU cores for optimal threading
            cpu_count = multiprocessing.cpu_count()
            # Use at least 4 threads, but leave 1 core free
            optimal_threads = max(4, min(cpu_count - 1, 8))
            ...
            print(
                colored(
                    f"Error creating default background: {str(e)}",
                    "red"))
            return None
    ...
    def create_section_clip(self, index, title, content, total_sections):
        ...
        try:
            print(
                colored(
                    f"\nProcessing section {index}/{total_sections}",
                    "yellow"))
            ...
            print(
                colored(
                    f"✓ Section {index} complete (duration: {duration:.1f}s)",
                    "green"))
        ...
        try:
            ...
            for file in glob.glob("temp/segments/*.mp4"):
                try:
                    ...
                    print(
                        colored(
                            f"Warning: Could not remove {file}: {str(e)}",
                            "yellow"))
            ...
            print(
                colored(
                    f"Warning: Error cleaning up temporary files: {str(e)}",
                    "yellow"))

    def cleanup_video_library(
            self,
            channel_type=None,
            max_videos=20,
            days_to_keep=30):
        ...
        try:
            ...
            if channel_type:
                ...
                if not channel_dirs:
                    log_warning(
                        f"No video directories found for {channel_type}")
                    return

                # Clean each directory
                for directory in channel_dirs:
                    cleaned = self._cleanup_directory(
                        directory, max_videos, days_to_keep)
                    if cleaned > 0:
                        total_cleaned += cleaned
                        categories_cleaned += 1

                log_success(
                    f"Cleaned up {total_cleaned} videos from {categories_cleaned} categories for {channel_type}")
                return
            ...
            for directory in categories:
                if os.path.isdir(directory):
                    try:
                        ...
                        log_warning(
                            f"Error cleaning {os.path.basename(directory)}: {str(e)}")

            if total_cleaned > 0:
                log_success(
                    f"Cleaned up {total_cleaned} videos from {categories_cleaned} categories")
            else:
        ...
        try:
            ...
            for file in os.listdir(directory):
                if file.endswith(".mp4"):
                    ...
                    videos.append({
                        ...
                        'created': created,
                        # Age in days
                        'age': (time.time() - created) / (60 * 60 * 24),
            ...
            for i, video in enumerate(reversed(videos)):
                if i < max_videos:
                    ...
                    # For older videos, keep them if they're within
                    # days_to_keep
            ...
            for video in to_delete:
                try:
                    ...
                    log_warning(
                        f"Could not remove {os.path.basename(video['path'])}: {str(e)}")
            ...
            log_warning(
                f"Error cleaning up directory {os.path.basename(directory)}: {str(e)}")
            return 0
    ...
    def music_volume(self, value):
        ...
        if not isinstance(value, (int, float)):
            self.logger.warning(
                f"Invalid music volume type: {type(value)}. Using default 0.3")
            self._music_volume = 0.3
        elif value < 0.0 or value > 1.0:
            self.logger.warning(
                f"Music volume {value} out of range (0.0-1.0). Clamping to valid range.")
...
def safe_mix_audio(voice_path, music_path, output_path, music_volume=0.3):
    """
    Safely mix audio files using pydub instead of MoviePy's CompositeAudioClip
    to avoid the 'fps' attribute error.
    ...
    try:
        ...
        # Apply fade effects
        fade_in_ms = 2000  # 2 seconds
        fade_out_ms = 3000  # 3 seconds
        ...
        print(colored(f"Error in safe audio mixing: {str(e)}", "red"))
        traceback.print_exc()
        
        # Fallback to just copying the voice file
        try:
            import shutil
